...
func (b *MTLSAuthenticatorBuilder) findCaCertificate() error {
	// Read and parse the passed certificate file which should contain the CA certificate and chain
	buf, err := os.ReadFile(b.caCertificatePath)
	if err != nil {
		return err
	}
	...
	if len(chainBlocks) <= 0 {
		return fmt.Errorf("didn't find certificate in file at path %s", b.caCertificatePath)
	}
